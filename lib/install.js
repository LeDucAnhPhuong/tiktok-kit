...
const { connectOfficial, selectLanguage } = require('./connect.js');
...
async function runInit({ global, connect = true, variant = 'flat', lang = null, scope = 'user', auth = true }) {
  ...
  return connectOfficial({ variant, scope, auth });
}
...
function runStatus({ global }) {
  ...
  console.log(`  output language : ${config.locale?.responseLanguage === 'en' ? 'English' : 'Tiếng Việt'}`);
  return 0;
}
