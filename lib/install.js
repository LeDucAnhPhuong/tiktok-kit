...
const { runConnect } = require('./connect.js');
...
async function runInit({ global, connect = true }) {
  ...
  if (!connect) {
    console.log('');
    console.log('Next: run `tk connect` to configure MCP access.');
    return 0;
  }

  if (alreadyConfigured()) {
    console.log('');
    console.log('MCP access already configured. Run `tk connect` to change vendor or rotate a key.');
    return 0;
  }
