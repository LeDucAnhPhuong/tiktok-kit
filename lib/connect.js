...
/**
 * Register the server with Claude Code's own CLI when it is available.
 *
 * User scope avoids the approval prompt that Claude Code puts in front of every
 * project-scoped .mcp.json server, which is one less manual step after install. It also
 * means never hand-editing ~/.claude.json, which holds a lot of Claude Code state that
...
function addViaClaudeCli(name, url, scope) {
  try {
    execFileSync(
      'claude',
      ['mcp', 'add', '--transport', 'http', name, url, '--scope', scope],
...
async function selectLanguage({ interactive = true, lang = null } = {}) {
  ...
  if (!chosen && interactive && process.stdin.isTTY) {
    const rl = makeInterface();
    try {
      console.log('\nLanguage for skill and agent output:');
      console.log('  1. Tiếng Việt (default)');
      console.log('  2. English');
      const raw = await ask(rl, 'Choose 1-2 [1]: ');
      chosen = raw === '2' ? 'en' : 'vi';
    } finally {
      rl.close();
    }
 ...
 */
function connectOfficial({ variant = 'flat', scope = 'user' } = {}) {
  ...
  const steps = ['Restart Claude Code'];
  if (needsApproval) {
    steps.push('Approve the project MCP server when Claude Code asks\n     (project-scoped servers are gated; `tk init --scope user` skips this)');
  }
  steps.push(
    'Run  /mcp  → pick "tiktok-ads" → Authenticate\n' +
      '     A browser opens: sign in to TikTok Ads Manager, review the permissions,\n' +
      '     click Authorize. Claude Code runs this handshake itself and keeps the\n' +
      '     token, which is why the CLI cannot do it for you. Skip it and the server\n' +
      '     is connected but unauthorized — every plane reads empty.'
  );
  steps.push('Run  /tk:connect  to verify data is flowing');

  console.log('Next:');
  steps.forEach((s, i) => console.log(`  ${i + 1}. ${s}`));

  console.log('');
  console.log('Authorization lasts 30 days — re-run /mcp when a live plane goes quiet.');
...
module.exports = { runConnect, connectOfficial, selectLanguage };
